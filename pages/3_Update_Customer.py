...
try:
    ...
    for i, (col, val) in enumerate(row_data.items()):
        with columns[i % 4]:
            if col == "status":
                edited_data[col] = st.selectbox(f"{col}", [0, 1], index=[0, 1].index(val) if val in [0, 1] else 0)
            else:
